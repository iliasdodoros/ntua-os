...
void compute_and_output_mandel_line(int fd, int line, int N) {
  ...
  for (j = 0; j < x_chars; j++)
    buffer[x_chars * line + j] = color_val[j];
...
int main(int argc, char **argv) {
  ...
  buffer = create_shared_memory_area(sizeof(int) * x_chars * y_chars);
  ...
  for (i = 0; i < y_chars; ++i)
    output_mandel_line(1, buffer + x_chars * i);

  destroy_shared_memory_area(buffer, sizeof(int) * x_chars * y_chars);
  reset_xterm_color(1);
  return 0;
}